...
class Client:
    ...
    def countdown_cancel_all_open_orders(self, countdownTime,symbol: str):
        """
        DELETE
        """
        req = "countdownCancelAll?"
        querystring = urllib.parse.urlencode(
            {
                "symbol": symbol,
    ...
    def change_leverage(self, leverage, symbol):
        """
        POST
    ...
    def position_info(self,symbol):
        ...
        req = "positionRisk?"
        querystring = urllib.parse.urlencode(
            {"recvWindow": self.recvWindow, "timestamp": self.timestamp(),"symbol": symbol}
        )
